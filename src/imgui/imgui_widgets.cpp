...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImU32 active_col = GetColorU32(ImGuiCol_FrameBgActive);

    RenderFrame(check_bb.Min, check_bb.Max, bg_col, true, style.FrameRounding);
    if (*v)
    {
        const float pad = square_sz * 0.25f;
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), active_col, style.FrameRounding);
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
    }
