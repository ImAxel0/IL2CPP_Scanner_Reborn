...
namespace IL2CPP
{
	namespace Class
	{
        ...
        Unity::il2cppPropertyInfo* GetProperties(Unity::il2cppClass* m_pClass, void** m_pIterator)
        {
            return reinterpret_cast<Unity::il2cppPropertyInfo*(IL2CPP_CALLING_CONVENTION)(void*, void**)>(Data.Functions.m_pClassGetProperties)(m_pClass, m_pIterator);
        }

        void FetchProperties(Unity::il2cppClass* m_pClass, std::vector<Unity::il2cppPropertyInfo*>* m_pVector, void* m_pPropertiesIterator)
        {
            m_pVector->clear();

            while (1)
            {
                Unity::il2cppPropertyInfo* m_pProperty = GetProperties(m_pClass, &m_pPropertiesIterator);
                if (!m_pProperty)
                    break;

                m_pVector->emplace_back(m_pProperty);
            }
        }
